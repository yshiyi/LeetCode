...
/*
Method: Create a hashmap. The key is the string, and the value is the number of counts of that string.
        Use string.find() and string.substr to truncate the string into parts.
        Time complexity: O(N), N is the length of cpdomains
...
public:
    vector<string> subdomainVisits(vector<string>& cpdomains) {
        ...
        for(auto entry:cpdomains){
            int sp = entry.find(' ');
            int count = stoi(entry.substr(0, sp));
            string domain = entry.substr(sp+1);
            m[domain] += count;
            int dp = domain.find('.');
            while(dp!=-1){
                domain = domain.substr(dp+1);
                m[domain] += count;
                dp = domain.find('.');
            }
        }
        ...
        vector<string> res;
        for(auto key:m){
            string str = to_string(key.second) + " " + key.first;
            res.push_back(str);
        }
        return res;
    }
